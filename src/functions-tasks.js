...
function retry(func, attempts) {
  let copyAttempts = attempts;
  const tryer = () => {
    let output;
    for (let i = 0; i < copyAttempts; i += 1) {
      try {
        if (func()) {
          output = func();
          copyAttempts -= 1;
...
function partialUsingArguments(fn, ...args1) {
  const outputerFunc = (...inputPrm) => {
    const res = fn(...args1, ...inputPrm);
    return res;
  };
  return outputerFunc;
}
...
function getIdGeneratorFunction(startFrom) {
  const objData = {
    callTimes: startFrom,
    showValue: function addOne() {
      this.lastVal = this.callTimes;
      this.callTimes += 1;
      return this.lastVal;
    },
  };
  return objData.showValue.bind(objData);
}
